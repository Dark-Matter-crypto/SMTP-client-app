...
public class MailPane extends StackPane {
    ...
    public MailPane(){
        ...
        emailSender.setOnAction((ActionEvent e) -> {
            if (fromAddressForm.getText().isEmpty() || toAddressForm.getText().isEmpty() || messageForm.getText().isEmpty()){
                ...
                toAddress = toAddressForm.getText();
                fromAddress = fromAddressForm.getText();
                ccAddress = ccAddressForm.getText();
                subject = subjectForm.getText();
                message = messageForm.getText();

                smtpClient.sendMail(fromAddress, toAddress, ccAddress, subject, message);
                response.setTextFill(Color.web("#228B22", 1));
                response.setText("Email successfully sent to: " + toAddress);
            }
        });
